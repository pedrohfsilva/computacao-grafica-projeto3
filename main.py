...
Cena: uma sala de aula durante uma greve universitária, com dois ambientes.
  - Interno: professor, alunos e mesas. Luzes: lâmpada do teto e celular.
  - Externo: grevistas, guarda, caixa de som e um carro com faróis.

Iluminação (Phong) com escopo: as luzes internas só iluminam objetos internos
e a luz do carro só ilumina objetos externos. O céu permanece sempre claro,
pois é dia (não é afetado pela iluminação).
...
# ====================================================================
# Estado (interruptores de luz, intensidades e interações da cena)
# ====================================================================

class State:
    def __init__(self):
        # Interruptores das luzes (requisito 3)
        self.car_light = True
        self.lamp_light = True
        self.phone_light = True
        self.ambient_on = True
        # Intensidades ajustáveis (requisitos 4, 5 e 6)
        self.ambient = 0.30
        self.diffuse = 1.0
        self.specular = 1.0
        # Interações da cena
...
def make_key_callback(state):
    """Eventos discretos: interruptores de luz e wireframe."""
...
def scene_input(window, state, dt):
    """Eventos contínuos: interações da cena e ajuste de reflexão."""
    def held(k):
...
# Cores das fontes de luz (as duas internas têm cores diferentes — requisito 2)
CAR_COLOR = glm.vec3(1.0, 0.96, 0.88)    # branco quente (faróis)
LAMP_COLOR = glm.vec3(1.0, 0.85, 0.55)   # amarelo quente (lâmpada do teto)
PHONE_COLOR = glm.vec3(0.35, 0.5, 1.0)   # azul (tela do celular)
...
def build_scene():
    ...
    # --- Estrutura da sala (interna). Superfícies foscas: ks baixo. ---
    s["floor_indoor"] = GameObject(plane, color=gray)
    ...
    s["ceiling"] = GameObject(cube, color=roof)   # laje fina: face de baixo voltada à sala
    ...
    # --- Ambiente externo: chão de grama e céu ---
    s["floor_outdoor"] = GameObject(plane, color=grass, exterior=True)
    ...
    s["skybox"].unlit = True   # o céu é sempre claro

    # --- Porta (entre os ambientes) ---
    ...
    door.center = glm.vec3(-45.47, 0.0, 0.0)   # dobradiça
    ...
    # --- Grevistas (externos, dois modelos) ---
    ...
    phone = GameObject(model("smartphone", "smartphone.obj"), model("smartphone", "smartphone.png"),
                       clamp_tex=True, flip_y=False)
    phone.scale = glm.vec3(0.08)
    phone.pos = glm.vec3(0.3, 0.82, -12.0)
    phone.rot = glm.vec3(-1.5708, 0, 0)   # deitado na mesa, tela para cima
    ...
    # Esferas que representam os faróis (acendem junto com a luz do carro)
...
# Posições reutilizadas para os objetos repetidos
...
def draw_frame(shader, s, car, camera, state, projection):
    """Define os uniforms do frame e desenha toda a cena."""
    ...
    # Luz 0: faróis do carro (spotlight externo, segue o carro)
    ...
    # Interações controladas pelo teclado
    s["hat"].scale = glm.vec3(state.hat_scale)
    s["prof"].pos.x = state.prof_x
    s["door"].rot.y = state.door_angle

    # Céu primeiro, sem teste de profundidade (fica ao fundo de tudo)
    ...
    # Grevistas (alterna os dois modelos)
    for obj, pos in [
    ...
    # Faróis (esferas que brilham) acompanham o carro
